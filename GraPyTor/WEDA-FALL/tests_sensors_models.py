...
SENSOR_ORDER = ["accel", "gyro", "orientation", "vertical"]
...
def sensor_groups(cols):
    groups = {k: [] for k in SENSOR_ORDER}
    for c in cols:
        cl = c.lower()
        if cl.startswith("accel"):
            groups["accel"].append(c)
        if cl.startswith("gyro"):
            groups["gyro"].append(c)
        if cl.startswith("orientation"):
            groups["orientation"].append(c)
        if cl.startswith("vertical"):
            groups["vertical"].append(c)
    for k in groups:
...
def selected_columns_from_mask(groups, mask):
    cols = []
    for i, g in enumerate(SENSOR_ORDER):
        if (mask >> i) & 1:
...
def main():
    ...
    groups = sensor_groups(numeric_feature_columns(train_df))
    ...
    pbar = tqdm(total=len(valid_masks) * 3, desc="Entraînements GPU", unit="step")

    for mask in valid_masks:
        bits = bitstring(mask)
        cols = selected_columns_from_mask(groups, mask)
