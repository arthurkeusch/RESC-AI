...
public class ObjectBox {
     ...
     * @param fileName the name of the file the chunk belongs to
     */
    public void putItem(String chunk, float[] embedding, String fileName) {

        Item item = new Item(chunk, embedding, fileName);
        itemBox.put(item);
    }

    /**
     * Get all unique file names stored in the database.
     * @return a list of unique file names
     */
    public List<String> getAllFiles() {
        String[] files = itemBox.query().build().property(Item_.fileName).distinct().findStrings();
        return java.util.Arrays.asList(files);
    }

    /**
     * Remove all items associated with a specific file name.
     * @param fileName the name of the file to remove
     */
    public void removeFile(String fileName) {
        itemBox.query().equal(Item_.fileName, fileName, QueryBuilder.StringOrder.CASE_SENSITIVE).build().remove();
    }
